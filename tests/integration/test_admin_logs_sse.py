...
from pivma.ai.contracts import OperationalEventIndex
...
async def make_admin_user(session):
    ...
    session.add_all(
        [
            AccessProfilePermission(
                profile_id=profile.id, permission_id=p.id
            )
...
async def test_sse_operational_and_ai_broadcaster():
    # Testar que os geradores do broadcaster emitem dados no formato SSE
