...
async def test_form_ai_evaluation_endpoint(client, session):
    ...
    # 2. Obter a form_instance criada na atividade proposal_submission
    ...
    # 3. Marcar um dos campos como ai_evaluation_enabled no banco
    ...
    # 4. Salvar rascunho com conteúdo para o campo
    client.put(
        f"/processes/{process_id}/activities/proposal_submission/form",
        headers=TRUSTED_ORIGIN,
        json={
            "values": {
                "scientific_justification": "Proposta detalhada de método alternativo para avaliação de toxicidade cutânea.",
            }
        },
    )

    # 5. Acionar avaliação por IA via POST /forms/instances/{instance_id}/evaluate-ai
    eval_resp = client.post(
        f"/forms/instances/{form_instance_id}/evaluate-ai",
        headers=TRUSTED_ORIGIN,
    )
