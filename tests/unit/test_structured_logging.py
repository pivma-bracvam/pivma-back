...
def test_setup_logging_configures_rotating_handlers(tmp_path: Path):
    ...
    op_handler = next(
        h for h in op_logger.handlers if isinstance(h, TimedRotatingFileHandler)
    )
    ai_handler = next(
        h for h in ai_logger.handlers if isinstance(h, TimedRotatingFileHandler)
    )

    assert op_handler.backupCount == 7
    assert ai_handler.backupCount == 7
...
def test_operational_logger_emits_valid_jsonl():
    ...
    assert last_line["duration_ms"] == 42.5
...
def test_ai_logger_emits_valid_jsonl():
    ...
    assert last_line["step_duration_ms"] == 15.2
    assert last_line["tokens_used"] == 120
    assert "timestamp" in last_line


def test_handler_rollover_retention_limit(tmp_path: Path):
    """Verifica se o TimedRotatingFileHandler respeita o backupCount ao rotacionar."""
