...
**Classificação**: **CONFIRMADO**: decisão do responsável da demanda determina exclusão física em cascata para rascunho nunca submetido, desistência de revisão devolvida, cancelamento e arquivamento sem justificativa de ciclo de vida. **CONFIRMADO**: endpoints REST são separados e cancelamento/arquivamento usam `triage.review`. **INFERÊNCIA**: o diretório local `ATTACHMENTS_DIR/{process_id}/` reúne os binários de anexo do processo.
...
- [X] T004 Criar teste unitário para revisão devolvida ser desistível, mas não deletável, em `tests/unit/core/test_process_retirement.py`
- [X] T005 Criar teste unitário para triage reviewer não poder deletar rascunho em `tests/unit/core/test_process_retirement.py`
- [X] T006 Implementar constantes de operações e status terminais sem justificativa em `src/pivma/core/process_engine.py`
...
- [X] T009 Implementar schemas de resposta dos endpoints separados em `src/pivma/schemas.py`
- [X] T010 Implementar projeção de `available_actions` com `WITHDRAW` em `src/pivma/schemas.py`

**Checkpoint**: O domínio identifica rascunho inicial e revisão devolvida sem ambiguidade; os schemas não aceitam corpo de justificativa para o ciclo de vida.

---

## Phase 3: User Story 1 - Excluir permanentemente um rascunho não submetido (Priority: P1) 🎯 MVP

**Goal**: Permitir somente ao proponente efetivo remover fisicamente o agregado de um rascunho que nunca entrou no pipeline.

**Independent Test**: Criar um rascunho inicial com dados e anexo, removê-lo sem justificativa e confirmar que não restam registros vinculados nem diretório de anexos.

### Tests for User Story 1

- [X] T011 [US1] Criar teste de API para `DELETE /processes/{id}` do proponente efetivo retornar 204 em `tests/api/routers/test_process_retirement.py`
- [X] T012 [US1] Criar teste de integração para `DELETE /processes/{id}` remover processo e registros vinculados do banco em `tests/integration/database/test_process_retirement_deletion.py`
...
- [X] T014 [US1] Criar teste de integração para `DELETE /processes/{id}` remover arquivo e diretório do processo e falhar sem apagar linhas se a remoção do arquivo falhar em `tests/integration/database/test_process_retirement_deletion.py`
- [X] T015 [US1] Criar teste de segurança para usuário com `triage.review` não excluir rascunho alheio em `tests/api/routers/test_process_retirement.py`
...
- [X] T021 [US1] Expor `DELETE /processes/{id}` com resposta `204 No Content` em `src/pivma/routers/processes.py`
...
**Goal**: Permitir que usuário com `triage.review` cancele um processo submetido, interrompa trabalho pendente e preserve o histórico.
...
- [X] T025 [US2] Criar teste de API para `CANCEL` não preencher justificativa em `closure_reason` ou auditoria em `tests/api/routers/test_process_retirement.py`
...
- [X] T034 [US2] Criar teste de segurança para usuário sem `triage.review` receber recusa ao cancelar em `tests/api/routers/test_process_retirement.py`
...
- [X] T043 [US2] Implementar `PATCH /cancellation` atômico com autorização `triage.review` e bloqueio de linha em `src/pivma/core/process_engine.py`
- [X] T044 [US2] Cancelar filhos não terminais e preservar filhos terminais no ramo `CANCEL` de `src/pivma/core/process_engine.py`
- [X] T045 [US2] Registrar `PROCESS_CANCELLED` com estados e contagens de filhos, sem justificativa, em `src/pivma/core/process_engine.py`
...
## Phase 5: User Story 3 - Desistir de revisão devolvida (Priority: P1)

**Goal**: Permitir que o proponente encerre uma nova elaboração após `REVISION_REQUESTED`, preservando todas as versões e decisões anteriores.

- [X] T088 [US3] Criar teste de API para `PATCH /withdrawal` do proponente retornar `CANCELLED` e auditoria específica em `tests/api/routers/test_process_retirement.py`
- [X] T089 [US3] Criar teste de API para rascunho inicial rejeitar `PATCH /withdrawal` em `tests/api/routers/test_process_retirement.py`
- [X] T090 [US3] Criar teste de segurança para revisor não executar desistência do proponente em `tests/api/routers/test_process_retirement.py`
- [X] T091 [US3] Implementar `withdraw_process` com cascata seletiva, auditoria sem justificativa e bloqueio de linha em `src/pivma/core/process_engine.py`
- [X] T092 [US3] Expor `PATCH /processes/{id}/withdrawal` em `src/pivma/routers/processes.py`

**Checkpoint**: O proponente consegue desistir de uma revisão devolvida sem apagar o histórico.

---

## Phase 6: User Story 4 - Arquivar um processo terminal (Priority: P2)

**Goal**: Permitir que usuário com `triage.review` arquive processos encerrados sem alterar seu histórico.
...
- [X] T057 [US4] Criar teste de segurança para usuário sem `triage.review` receber recusa ao arquivar em `tests/api/routers/test_process_retirement.py`
...
- [X] T059 [US4] Criar teste de API para `GET /processes?status=ARCHIVED` retornar arquivo para usuário com `triage.review` em `tests/api/routers/test_process_retirement.py`
- [X] T060 [US4] Criar teste de segurança para consulta `status=ARCHIVED` sem `triage.review` retornar `forbidden` em `tests/api/routers/test_process_retirement.py`
- [X] T061 [US4] Criar teste de API para edição de formulário de processo arquivado retornar erro de transição em `tests/api/routers/test_process_retirement.py`
- [X] T062 [US4] Criar teste de API para linha do tempo de processo arquivado expor o evento de arquivamento em `tests/api/routers/test_process_retirement.py`

### Implementation for User Story 4

- [X] T063 [US4] Implementar `PATCH /archive` com autorização `triage.review` em `src/pivma/core/process_engine.py`
- [X] T064 [US4] Registrar `PROCESS_ARCHIVED` com status anterior, sem justificativa, em `src/pivma/core/process_engine.py`
- [X] T065 [US4] Omitir `ARCHIVED` quando `GET /processes` não recebe filtro de status em `src/pivma/routers/processes.py`
- [X] T066 [US4] Autorizar a consulta explícita `status=ARCHIVED` somente para `triage.review` em `src/pivma/routers/processes.py`

**Checkpoint**: Arquivamento separa histórico de operação sem excluir dados ou permitir novas mutações.

---

## Phase 7: User Story 5 - Front-end conhece as ações permitidas (Priority: P3)

**Goal**: Entregar códigos de ação e falha estáveis para a interface sem transferir a autorização ao cliente.

**Independent Test**: Consultar os mesmos processos como proponente e como usuário com `triage.review` e comparar `available_actions` com o resultado da operação enviada ao backend.

### Tests for User Story 5

- [X] T067 [US5] Criar teste de API para rascunho próprio expor `DELETE_DRAFT` em `available_actions` em `tests/api/routers/test_process_retirement.py`
- [X] T068 [US5] Criar teste de API para revisão devolvida expor `WITHDRAW` em `available_actions` em `tests/api/routers/test_process_retirement.py`
- [X] T069 [US5] Criar teste de API para processo ativo expor `CANCEL` ao usuário com `triage.review` em `tests/api/routers/test_process_retirement.py`
- [X] T070 [US5] Criar teste de API para processo `CLOSED` expor `ARCHIVE` ao usuário com `triage.review` em `tests/api/routers/test_process_retirement.py`
- [X] T071 [US5] Criar teste de API para processo arquivado expor lista vazia de ações em `tests/api/routers/test_process_retirement.py`
- [X] T073 [US5] Criar teste de API para processo invisível retornar código `not_found` em `tests/api/routers/test_process_retirement.py`
- [X] T074 [US5] Criar teste de API para operação enviada fora de `available_actions` retornar código `invalid_transition` em `tests/api/routers/test_process_retirement.py`

### Implementation for User Story 5

- [X] T075 [US5] Implementar cálculo de `available_actions` com estado, eventos de submissão/devolução e autorização em `src/pivma/core/process_engine.py`
- [X] T076 [US5] Incluir `available_actions` nas projeções de detalhe e listagem em `src/pivma/routers/processes.py`
- [X] T077 [US5] Padronizar códigos `not_found`, `forbidden` e `invalid_transition` em `src/pivma/routers/processes.py`

**Checkpoint**: A interface recebe apenas ações aplicáveis e respostas de falha previsíveis; o backend revalida cada comando.

---

## Phase 8: Demonstração, seed e validação transversal
...
```text
Preparação → Fundamentos → US1 (MVP) → US2 → US3 (desistência) → US4 (arquivo) → US5 → Demonstração e validação
...
- US3 acrescenta desistência do proponente após devolução, preservando o histórico.
- US4 acrescenta arquivamento e a consulta histórica após a base de cancelamento.
- US5 projeta ações permitidas e padroniza os códigos depois que as transições existem.
