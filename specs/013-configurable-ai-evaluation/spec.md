...
- **FR-022**: O sistema MUST executar internamente as etapas de preparação de dados, extração de evidências, avaliação dos critérios, síntese e geração do relatório, mantendo essa orquestração escondida atrás de um motor de pipeline único. **Débito fechado na Spec 014**: a esteira mock legada da Spec 010 (`FormAIPipelineEngine`, `ai/steps/*`, endpoint `/forms/instances/{id}/evaluate-ai`, `_run_legacy_field_ai_mock`) foi **removida**; o `EvaluationPipeline` é o único motor. O achado L1 do `/speckit-analyze` fica encerrado.
