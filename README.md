...
| Excluir rascunho inicial | `DELETE /processes/{id}` | Remove fisicamente apenas o processo nunca submetido, pelo próprio proponente |
| Desistir de revisão devolvida | `PATCH /processes/{id}/withdrawal` | Encerra como `CANCELLED` uma nova elaboração do proponente, preservando o histórico |
| Cancelar processo | `PATCH /processes/{id}/cancellation` | Interrompe processo submetido para usuário com `triage.review` |
