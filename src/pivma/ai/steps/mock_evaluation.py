...
class MockEvaluationStep:
    ...
    def __init__(self) -> None:
        self.step_order = 2
        self.step_name = 'mock_evaluation'
    ...
    def execute(self, context: PipelineContext) -> StepResult:
        ...
        if not sanitized_value:
            issues.append(
                'Campo obrigatório ou sem conteúdo para análise técnica.'
            )
        else:
            issues.append(
                'A justificativa técnica não apresenta dados comparativos '
                'de citotoxicidade prévia.'
            )
            issues.append(
                'Ausência de detalhamento quanto aos controles '
                'metodológicos (positivos e negativos) aplicados.'
            )
