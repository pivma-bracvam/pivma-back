...
class VerdictSynthesisStep:
    ...
    def __init__(self) -> None:
        self.step_order = 3
        self.step_name = 'verdict_synthesis'
    ...
    def execute(self, context: PipelineContext) -> StepResult:
        ...
        if not issues:
            issues = ['Pendências técnicas identificadas no preenchimento.']

        recommendations = [
            (
                'Incluir referências bibliográficas de estudos de '
                'viabilidade com metodologias correlatas.'
            ),
            (
                'Descrever explicitamente os critérios de aceitação e '
                'limites de detecção do ensaio.'
            ),
        ]
